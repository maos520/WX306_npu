...
(
 ...
 // axi write address channel
   ...
   input      [      8-1: 0] axi_awlen_i    ,  //!< AXI write burst length
 ...
 // axi read address channel
   ...
   input      [      8-1: 0] axi_arlen_i    ,  //!< AXI read burst length
...
reg read_start_r;
wire rstart;
assign wr_errorw = (axi_awlen_i != 8'h0) || (axi_awsize_i != 3'b011); // error if write burst and more/less than 4B transfer
assign rd_errorw = (axi_arlen_i != 8'h0) || (axi_arsize_i != 3'b011); // error if read burst and more/less than 4B transfer
//assign wr_errorw = 1'b0; // error if write burst and more/less than 4B transfer
//assign rd_errorw = 1'b0; // error if read burst and more/less than 4B transfer
...
always @(posedge axi_clk_i) begin
   ...
   else if (rd_do && (cnt!=16'b0) )begin
      cnt  <= cnt-1'b1 ;
   end

end

assign rstart= read_start && (~read_start_r);
assign read_start=axi_arvalid_i && axi_arready_o;
always @(posedge axi_clk_i) begin
   if (axi_rstn_i == 1'b0) begin
      read_start_r    <= 1'b0 ;
   end
   else begin
      read_start_r<=read_start;
   end
end
assign sys_ren=(axi_rready_i&&(cnt2!=16'b0))? 1'b1:1'b0;
always @(posedge axi_clk_i) begin
   if (axi_rstn_i == 1'b0) begin
      cnt2    <= 16'b0 ;
   end
      ...
      cnt2  <= axi_arlen_i+1'b1 ;
   end

   else if (axi_rready_i && (cnt2!=16'b0) )begin
      cnt2  <= cnt2-1'b1 ;
   end
...
assign axi_rlast_o  = rd_do_r&&!rd_do;
